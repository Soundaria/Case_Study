	...
	public class AdminController : ControllerBase
	{
		...
		public IActionResult ValidatingAdmin([FromBody] Admin admin)
		{
			try
			{
				...
				Admin id = user.Find(e => (e.Email == admin.Email) && (e.Password == admin.Password));
				if (id != null)
				{
					var jwt = jWTManagerRepository.AuthenticateAdmin(id.Email, id.Password);
		...
		public IActionResult AddAdmin([FromBody] Admin admin)
		{
			try
			{
				...
				return Created("Admin added", admin);
			}
			catch (NullReferenceException ex)
			{
				logger.LogWarning("Data not found" + ex.Message);
		...
		public IActionResult UpdateAdmin( [FromBody] Admin adminobj,int adminId)
		{
			try
			{
				//int adminIdtoken = Convert.ToInt32(HttpContext.User.FindFirstValue("adminId"));
				...
				if (admin != null)
				{
					...
					return StatusCode(200, "Admin details added successfully");
				}
				else
				{
		...
		public IActionResult DeleteAdmin(int adminId)
		{
			try
			{
				//int tokenadminId = Convert.ToInt32(HttpContext.User.FindFirstValue("adminId"));
				Admin admin = repo.Admin.Find(adminId);
		...
		//Adding Trainer
		[HttpPost("AddTrainer/{adminId}")]
		public IActionResult AddTrainer([FromBody] Trainer trainer,int adminId)
		{
			try
			{
		...
		public IActionResult GetTrainerbyAdmin()
		{
			...
			//Adding Courses
			[HttpPost("AddCourse/{adminId}")]
		public IActionResult AddCourse( [FromBody] Course course,int adminId)
		{
			try
			{
		...
		[HttpPost("AddCourseToTrainer/{adminId}")]
		public IActionResult AddCourseToTrainer([FromBody] TrainerCourse trainerCourse,int adminId)
		{
			try
			{
				//int admin = Convert.ToInt32(HttpContext.User.FindFirstValue("adminId"));
		...
		public IActionResult UpdateCourseToTrainer([FromBody] TrainerCourse trainerCourseobj, int Id)
		{
			try
			{
				...
				trainercourse.BatchName = trainerCourseobj.BatchName;
